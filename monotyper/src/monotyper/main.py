...
def _build_parser() -> argparse.ArgumentParser:
    ...
    render_parser = subparsers.add_parser("render", help="render source files to an output directory")
...
def run_render(input_path: Path, output_path: Path) -> int:
    ...
    stage_dir = make_stage_dir(output_path)
    try:
        if input_path.is_file():
            ...
            install_stage(stage_output, output_path)
        else:
    ...
    print(f"Wrote {written} output {noun} to {output_path}")
    return 0
        ...
        return FileResult(
            ...
            diagnostics=[
                Diagnostic(
                    ...
                    line=1,
                    column=1,
                    message="skipping file that disappeared during scan",
                )
            ],
            rendered_text="",
        )
